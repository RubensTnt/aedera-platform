  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Profilo DATI_WBS</h2>
          <p className="mt-1 text-sm text-slate-500">
            Definisci quali livelli WBS e quali codici sono richiesti per il progetto{" "}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-4 py-2 text-left font-medium text-slate-600">Livello</th>
              <th className="px-4 py-2 text-left font-medium text-slate-600">Etichetta</th>
              <th className="px-4 py-2 text-left font-medium text-slate-600">Attivo</th>
              <th className="px-4 py-2 text-left font-medium text-slate-600">Obbligatorio</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={key}>
                  ...
                  <td className="px-4 py-2 text-slate-800">
                    <input
                      type="text"
                      className="w-full rounded-md border border-slate-200 px-2 py-1 text-sm text-slate-800 disabled:bg-slate-50"
                      value={cfg.label}
                      disabled={disabled}
                      onChange={(e) =>
                        setProfile({
                          ...
                          levels: profile.levels.map((lvl) =>
                            lvl.key === key ? { ...lvl, label: e.target.value } : lvl,
                          ),
                        })
                      }
                    />
